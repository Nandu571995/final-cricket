...
def fetch_json_api(match_id):
    """Call Cricbuzz internal REST APIs. Returns dict or None."""
    result = {}
    for url in [
...
def parse_json_api(api, data):
    """Parse Cricbuzz mini-scorecard JSON into overlay format."""
    bat_names = []
    ms   = api.get("miniscore") or {}
    mhdr = api.get("matchHeader") or {}
    msd  = api.get("matchScoreDetails") or {}

    fmt = (mhdr.get("matchFormat") or "").upper()
    ...
    if not data["team1"]["name"]:
        t1 = mhdr.get("team1") or {}; t2 = mhdr.get("team2") or {}
    ...
    b1 = _mbat(ms.get("batsmanStriker") or ms.get("striker") or {})
    b2 = _mbat(ms.get("batsmanNonStriker") or ms.get("nonStriker") or {})
    ...
    if isinstance(bw, dict) and bw:
        ...
        if bname:
            ...
            except: beco = str(bw.get("bowlEcon") or "0.00")
